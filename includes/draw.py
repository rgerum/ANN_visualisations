...
class Image:
    ...
    def plotNodes(self, x, y, name, color="C0"):
        ...
        ax.set_aspect(1)
        if 1:
            obj.artist = plt.Circle((obj.x, obj.y), Node_Radius, facecolor=darken(color, -0.2), edgecolor=darken(color, 0.6), lw=2)
            ax.add_artist(obj.artist)
        else:
    ...
    def plotArrow(self, obj1, obj2, color=None, zorder=1):
        ...
        plt.arrow(pos_start[0], pos_start[1], delta[0], delta[1], head_width=0.1, head_length=0.1, fc=color, length_includes_head=True, color=color, zorder=zorder)
    ...
    def show(self):
        ...
        import matplotlib as mpl
        plt.figure(1).set_size_inches(15.200000/2.54, 4.000000/2.54, forward=True)
