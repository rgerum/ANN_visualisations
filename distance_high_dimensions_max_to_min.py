...
relative = []
ns = []
for n in np.geomspace(1, 1000, 100):
    n = int(n)
    #plt.subplot(3, 3, n-1)
...
plt.ylabel("relative distance nearest to fartherst")
#plt.legend()
#% start: automatic generated code from pylustrator
plt.figure(1).ax_dict = {ax.get_label(): ax for ax in plt.figure(1).axes}
import matplotlib as mpl
plt.figure(1).set_size_inches(7.980000/2.54, 6.000000/2.54, forward=True)
plt.figure(1).axes[0].set_position([0.223726, 0.198983, 0.704936, 0.770000])
plt.figure(1).axes[0].spines['right'].set_visible(False)
plt.figure(1).axes[0].spines['top'].set_visible(False)
plt.figure(1).axes[0].get_yaxis().get_label().set_text("relative distance\nnearest to fartherst")
#% end: automatic generated code from pylustrator
plt.savefig("distance_high_dimensions_max_to_min.png")
plt.show()
